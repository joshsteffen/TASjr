...
use bytemuck::{Pod, cast, from_bytes, from_bytes_mut};
...
impl Vm {
    ...
    pub fn cast_mem<T: Pod>(&self, address: u32) -> &T {
        from_bytes(&self.data[address as usize..][..size_of::<T>()])
    }

    pub fn cast_mem_mut<T: Pod>(&mut self, address: u32) -> &mut T {
        from_bytes_mut(&mut self.data[address as usize..][..size_of::<T>()])
    }
    ...
    pub fn step(&mut self) -> Option<ExitReason> {
        ...
        // println!("{}: {opcode:?} {arg}", self.pc);
        self.pc += 1;
        match opcode {
            Opcode::Enter => self.program_stack -= arg,
            Opcode::Leave => {
            ...
            Opcode::Load4 => {
                let address = self.op_stack.pop().unwrap();
                self.op_stack.push(self.read_mem::<u32>(address));
            }
            Opcode::Store1 => {
            ...
            Opcode::Lsh => self.binary_op(u32::shl),
            Opcode::Rshi => self.binary_op(i32::shr),
            Opcode::Rshu => self.binary_op(u32::shr),
